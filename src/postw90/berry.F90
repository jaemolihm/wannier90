...
contains
  ...
  subroutine berry_main
    ...
    if (eval_nlspin) then
      allocate (nlspin_k_list(3, 4, 3, 3, kubo_nfreq, 3))
      allocate (nlspin_list(3, 4, 3, 3, kubo_nfreq, 3))
      nlspin_k_list = 0.0_dp
      nlspin_list = 0.0_dp
    ...
    if (on_root) then
      ...
      if (eval_nlspin) then
        ...
        fac_nlspin_shift = -cmplx_i * pi / 2.d0 * eV_seconds * elem_charge_SI**3 / (hbar_SI**2 * cell_volume)
        fac_nlspin_inj = cmplx_i * pi * elem_charge_SI**3 / (hbar_SI**2 * cell_volume)
        fac_nlspin_fermi = 1.d0 / 2.d0 * eV_seconds * elem_charge_SI**3 / (hbar_SI**2 * cell_volume)

        nlspin_list(:, :, :, :, :, 1) = nlspin_list(:, :, :, :, :, 1) * fac_nlspin_shift
        nlspin_list(:, :, :, :, :, 2) = nlspin_list(:, :, :, :, :, 2) * fac_nlspin_inj
        nlspin_list(:, :, :, :, :, 3) = nlspin_list(:, :, :, :, :, 3) * fac_nlspin_fermi

        do ifreq = 1, kubo_nfreq
          nlspin_list(:, :, :, :, ifreq, 1) = nlspin_list(:, :, :, :, ifreq, 1) / kubo_freq_list(ifreq)**2
          nlspin_list(:, :, :, :, ifreq, 2) = nlspin_list(:, :, :, :, ifreq, 2) / kubo_freq_list(ifreq)**2
        enddo
...
!        rmn_save_jml_D(:, m, n, ik) = cmplx_i*D_h(m, n, :)
    ...
    if (use_pwf_jml) then
      call pw90common_fourier_R_to_k_vec(kpt, vel_r_pwf, OO_true=vel_k)
      do a = 1, 3
        vel_k(:, :, a) = utility_rotate(vel_k(:, :, a), UU, num_wann)
      enddo
      ! loop on initial and final bands
      do n = 1, num_wann
        do m = 1, num_wann
  ...
  subroutine berry_get_nlspin_klist(kpt, nlspin_k_list, ik)
    !====================================================================!
    !                                                                    !
    ...
    do n = 1, num_wann
      do m = 1, num_wann
        ...
        ! I_mn(a, ispin, b, c) = - (occ(m) - occ(n)) * delta_jvk(m, n, a, ispin) * HH_da(m, n, b) * HH_da(n, m, c)
        I_mn = cmplx_0
        do c = 1, 3
          do b = 1, 3
            I_mn(:, :, b, c) = -delta_jvk(m, n, :, :) * HH_da(m, n, b) * HH_da(n, m, c)
          enddo
        enddo
        I_mn = I_mn * occ_fac
...
! END JML perturbed Wannier functions -----------------------------------
  ...
  contains
    ...
    subroutine berry_get_js_k(kpt, eig, del_alpha_eig, D_alpha_h, UU, js_k)
      ...
      complex(kind=dp)    :: SR_k(num_wann, num_wann)
      complex(kind=dp)    :: SR_alpha_k(num_wann, num_wann)
      complex(kind=dp)    :: SHR_k(num_wann, num_wann)
      ...
      ! QZYZ18 Eq.(36)
      call pw90common_fourier_R_to_k_new(kpt, SS_R(:, :, :, shc_gamma), OO=S_k)
      ! QZYZ18 Eq.(30)
      call utility_rotate_new(S_k, UU, num_wann)

      !=========== K_k ===========
      ! < u_k | sigma_gamma | \partial_alpha u_k >, QZYZ18 Eq.(26)
      ! QZYZ18 Eq.(37)
      call pw90common_fourier_R_to_k_new(kpt, SR_R(:, :, :, shc_gamma, shc_alpha), OO=SR_k)
      ! QZYZ18 Eq.(31)
      call utility_rotate_new(SR_k, UU, num_wann)
      SR_alpha_k = -cmplx_i * SR_k
      K_k = SR_alpha_k + matmul(S_k, D_alpha_h)

      !=========== L_k ===========
      ! < u_k | sigma_gamma.H | \partial_alpha u_k >, QZYZ18 Eq.(27)
      ! QZYZ18 Eq.(38)
      call pw90common_fourier_R_to_k_new(kpt, SHR_R(:, :, :, shc_gamma, shc_alpha), OO=SHR_k)
      ! QZYZ18 Eq.(32)
      call utility_rotate_new(SHR_k, UU, num_wann)
      SHR_alpha_k = -cmplx_i * SHR_k
      ! QZYZ18 Eq.(39)
      call pw90common_fourier_R_to_k_new(kpt, SH_R(:, :, :, shc_gamma), OO=SH_k)
      ! QZYZ18 Eq.(32)
      call utility_rotate_new(SH_k, UU, num_wann)
